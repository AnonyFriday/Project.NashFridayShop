...
public sealed class Validator : AbstractValidator<Request>
{
    public Validator()
    {
        RuleFor(x => x.SearchName)
            .MaximumLength(100).WithMessage("Search has a maximum length of 100 characters.")
            .When(x => !string.IsNullOrWhiteSpace(x.SearchName));

        RuleFor(x => x)
            .Must(x => x.MinPrice <= x.MaxPrice).WithMessage("Min Price must be less than or equal to Max Price.")
