...
public class StoreDbContextSeeder(StoreDbContext dbContext, ILogger<StoreDbContextSeeder> logger)
{
    public async Task SeedAsync()
    {
        ...
        var productRatings = new List<ProductRating>
        {
            ...
            new ProductRatingBuilder().WithProductId(products[0].Id).WithStars(5).WithComment("Exceeded my expectations.").Build(),
            new ProductRatingBuilder().WithProductId(products[0].Id).WithStars(4).WithComment("Beautiful texture and very sturdy.").Build(),
            new ProductRatingBuilder().WithProductId(products[0].Id).WithStars(5).WithComment("Perfect for my home decor.").Build(),
            new ProductRatingBuilder().WithProductId(products[0].Id).WithStars(3).WithComment("Good, but the color is slightly different from the photo.").Build(),
            new ProductRatingBuilder().WithProductId(products[0].Id).WithStars(5).WithComment("Highly recommend this!").Build(),
            new ProductRatingBuilder().WithProductId(products[0].Id).WithStars(4).WithComment("Great value for money.").Build(),
            new ProductRatingBuilder().WithProductId(products[0].Id).WithStars(5).WithComment("Love the craftsmanship.").Build(),
            new ProductRatingBuilder().WithProductId(products[0].Id).WithStars(4).WithComment("Solid build and fast shipping.").Build(),
            new ProductRatingBuilder().WithProductId(products[0].Id).WithStars(5).WithComment("A piece of art!").Build(),
            new ProductRatingBuilder().WithProductId(products[0].Id).WithStars(5).WithComment("Will definitely buy again.").Build(),
            new ProductRatingBuilder().WithProductId(products[0].Id).WithStars(4).WithComment("Very happy with this purchase.").Build(),
            new ProductRatingBuilder().WithProductId(products[0].Id).WithStars(5).WithComment("Amazing quality bamboo!").Build(),
