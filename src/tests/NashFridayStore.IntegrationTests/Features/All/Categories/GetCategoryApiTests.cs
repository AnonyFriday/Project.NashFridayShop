...
public class GetCategoryApiTests : IClassFixture<CustomWebApplicationFactory>, IDisposable
{
    ...
    public async Task GetCategory_ById_ShouldReturnCategory()
    {
        ...
        // Act
        HttpResponseMessage response = await _client.GetAsync($"/api/all/categories/{category.Id}", cancellationToken);
    ...
    public async Task GetCategory_NonExistentId_ShouldReturnNotFound()
    {
        ...
        // Act
        HttpResponseMessage response = await _client.GetAsync($"/api/all/categories/{nonExistentId}", cancellationToken);
