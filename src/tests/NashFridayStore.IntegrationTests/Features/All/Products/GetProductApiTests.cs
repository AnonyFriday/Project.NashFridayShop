...
using NashFridayStore.API.Features.All.Products.GetProduct;
...
public class GetProductApiTests : IClassFixture<CustomWebApplicationFactory>, IDisposable
{
    ...
    public async Task GetProduct_ById_ShouldReturnProduct()
    {
        ...
        // Act
        HttpResponseMessage response = await _client.GetAsync($"/api/all/products/{product.Id}", cancellationToken);
