...
using NashFridayStore.API.Features.Admin.Categories.CreateCategory;
...
public class CreateCategoryApiTests : IClassFixture<CustomWebApplicationFactory>, IDisposable
{
    ...
    public async Task CreateCategory_ValidRequest_ShouldReturnCreated()
    {
        ...
        // Act
        HttpResponseMessage response = await _client.PostAsJsonAsync("/api/admin/categories", request, cancellationToken);
