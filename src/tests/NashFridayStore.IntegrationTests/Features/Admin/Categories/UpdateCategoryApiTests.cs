...
namespace NashFridayStore.IntegrationTests.Features.Admin.Categories;
...
public class UpdateCategoryApiTests : IClassFixture<CustomWebApplicationFactory>, IDisposable
{
    ...
    public async Task UpdateCategory_ValidRequest_ShouldReturnOk()
    {
        ...
        // Act
        HttpResponseMessage response = await _client.PutAsJsonAsync($"/api/admin/categories/{category.Id}", body, cancellationToken);
    ...
    public async Task UpdateCategory_NonExistentId_ShouldReturnNotFound()
    {
        ...
        // Act
        HttpResponseMessage response = await _client.PutAsJsonAsync($"/api/admin/categories/{nonExistentId}", body, cancellationToken);
    ...
    public async Task UpdateCategory_DuplicateName_ShouldReturnConflict()
    {
        ...
        // Act
        HttpResponseMessage response = await _client.PutAsJsonAsync($"/api/admin/categories/{category1.Id}", body, cancellationToken);
